...
한국에서 지금 관심이 커지는 작은 신호를 10~30분 단위로 살펴보는 Streamlit 전광판입니다. Google Trends 공개 RSS와 Google News 공개 RSS로 키워드를 찾고, NAVER API HUB 검색어 트렌드로 10대·20대 상대 반응을 보강합니다. 샘플은 사용자가 미리보기 스위치를 직접 켠 경우에만 별도로 표시되며 실데이터와 절대 섞이거나 저장되지 않습니다.
...
│  ├─ fetch_naver_datalab.py      # NAVER API HUB 10대·20대 상대 반응
...
- `sample_data/sample_trends.csv`: 왼쪽의 샘플 미리보기를 직접 켰을 때만 사용하는 별도 데이터
...
API 키가 없어도 Google 공개 RSS 실데이터 수집은 동작합니다. NAVER 키가 없으면 연령 반응만 비어 있습니다. 샘플 TOP 20은 왼쪽의 `샘플 데이터 미리보기`를 직접 켰을 때만 보입니다.
...
## 8. NAVER API HUB 키 발급

2026년 6월부터 Search Trend가 NAVER API HUB로 이관 중이므로 새 프로젝트는 API HUB 키 사용을 권장합니다.

1. `https://console.ncloud.com`에 로그인합니다.
2. 왼쪽 위 `Services` → `AI·NAVER API` → `AI·NAVER API`로 이동합니다.
3. 왼쪽 `Application` → `Application 등록`을 누릅니다.
4. 이름을 입력하고 서비스 선택에서 `Search Trend`를 선택합니다.
5. 웹 서비스 URL에는 로컬 테스트 시 `http://localhost:8501`, 배포 후에는 실제 `https://....streamlit.app` 주소를 등록합니다.
6. 등록 후 앱 목록에서 `인증 정보`를 누릅니다.
7. Client ID와 Client Secret을 복사해 Streamlit의 `Manage app` → `Settings` → `Secrets`에 다음처럼 저장합니다.

```toml
NAVER_API_HUB_CLIENT_ID = "복사한 Client ID"
NAVER_API_HUB_CLIENT_SECRET = "복사한 Client Secret"
```

키는 GitHub 파일에 넣지 않습니다. 앱의 10대 지수는 13~18세, 20대 지수는 19~29세 조건의 최근 3일 평균 상대지수입니다.
...
- 빈 화면: 첫 실행이라면 `실데이터 수동 갱신`을 누릅니다. 화면 확인만 원하면 샘플 미리보기를 직접 켭니다.
- RSS 실패: 인터넷 연결·회사 방화벽 확인. 수집 상태에서 이유를 확인합니다. 실패 시 기존 실데이터를 유지하며 샘플로 바꾸지 않습니다.
...
2. NAVER 연령별 상대지수의 변화 이력과 24시간 최고점 저장
...
샘플 데이터는 기능 확인용이며 실제 현재 순위를 뜻하지 않습니다. 샘플 미리보기는 실데이터 CSV와 시간 기록에 저장되지 않습니다.
